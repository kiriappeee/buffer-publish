...
import { makeUnicodeAwareIndexUnaware } from './stringUtils';

const parseLinksAndTwitterEntities = text =>
...
const parseFacebookEntities = (draftText, entities) => {
  if (!entities) return [];

  return entities.map(({ url, text, indices }) => ({
    displayString: text,
    indices: [
      makeUnicodeAwareIndexUnaware(draftText, indices[0]),
      makeUnicodeAwareIndexUnaware(draftText, indices[1]),
    ],
    rawString: text,
    url,
  }));
...
export { parseLinksAndTwitterEntities, parseFacebookEntities };
